...
func TestLoggingDefaultsStatusToOKWithoutWrite(t *testing.T) {
	var logs bytes.Buffer
	logger := slog.New(slog.NewJSONHandler(&logs, nil))
	handler := middleware.Logging(logger, http.HandlerFunc(func(http.ResponseWriter, *http.Request) {}))
...
func TestAccessLogResponseWriterUnwraps(t *testing.T) {
	...
	tracked := &middleware.AccessLogResponseWriter{ResponseWriter: base}
