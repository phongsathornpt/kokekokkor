...
func TestAnthropicMessagesToGemini(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("AnthropicMessagesToGemini() error = %v", err)
	}
...
func TestGeminiGenerateContentToAnthropic(t *testing.T) {
	...
	if len(output.Candidates) != 1 || len(output.Candidates[0].Content.Parts) != 1 || output.Candidates[0].Content.Parts[0].Text == nil || *output.Candidates[0].Content.Parts[0].Text != "hello" {
		t.Fatalf("response = %s", response.Body)
	}
}
